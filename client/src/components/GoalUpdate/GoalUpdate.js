...
const GoalUpdate = (props) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/${category}/${id}`) //TODO confirm axios path and add authorization
    .then( res => {
      setGoal(res.data);
      setLoaded(true);
    })
  ...
  const putSubmit = () => {
    axios.put(`http://localhost:5000/api/${category}/${id}`, goal) //TODO confirm axios path and add authorization
